...
$testRunId = 'verify_phase1_visibility_'.Carbon::now()->format('Y_m_d_His');
...
try {
    ...
    check('Schema has lost_sales table', Schema::hasTable('lost_sales'));
    check('Schema has location_daily_metrics table', Schema::hasTable('location_daily_metrics'));
    check('Schema has demand_events table', Schema::hasTable('demand_events'));
    check('Schema has alerts table', Schema::hasTable('alerts'));
    check('Schema has orders table', Schema::hasTable('orders'));
    check('Schema has order_items table', Schema::hasTable('order_items'));
    ...
    $forecastStore = Location::factory()->create(['type' => 'store', 'sell_price' => null]);
    Inventory::factory()->create([
        'user_id' => $user->id,
        'location_id' => $forecastStore->id,
        'product_id' => $product->id,
        'quantity' => 2,
    ]);
    ...
    $incomingQuantity = 20;
    $incomingDeliveryDay = $gameState->day + 2;
    ...
    $incomingOrder = Order::factory()->create([
        ...
        'location_id' => $forecastStore->id,
        'status' => 'shipped',
        'delivery_day' => $incomingDeliveryDay,
    ...
    $forecast = $forecastService->getForecast($user->id, $forecastStore->id, $product->id, $gameState->day);
    ...
    check(
        'Forecast day 1 is stockout risk with low starting stock (2 units)',
        ($forecast[0]['risk_level'] ?? null) === 'stockout',
        ['row' => $forecast[0] ?? null]
    );

    check(
        "Forecast includes incoming order deliveries on day {$incomingDeliveryDay}",
        ($forecast[1]['incoming_deliveries'] ?? null) === $incomingQuantity,
        ['row' => $forecast[1] ?? null]
    );
    echo "\n";
    ...
    $summaryListener = app(CreateDailySummaryAlert::class);
    ...
    check('Location model casts sell_price as integer', ((new Location)->getCasts()['sell_price'] ?? null) === 'integer');
    echo "\n";

    // ============ FRONTEND WIRING CHECKS ============
    echo "--- Frontend Wiring Checks ---\n";

    $chartComponent = base_path('resources/js/components/DemandForecastChart.tsx');
    $skuDetailPage = base_path('resources/js/pages/game/sku-detail.tsx');
    $dashboardPage = base_path('resources/js/pages/game/dashboard.tsx');

    check('DemandForecastChart component file exists', is_file($chartComponent));
    check('SKU detail page exists', is_file($skuDetailPage));
    check('Dashboard page exists', is_file($dashboardPage));

    $chartSource = is_file($chartComponent) ? (string) file_get_contents($chartComponent) : '';
    $skuSource = is_file($skuDetailPage) ? (string) file_get_contents($skuDetailPage) : '';
    $dashboardSource = is_file($dashboardPage) ? (string) file_get_contents($dashboardPage) : '';

    check('Chart uses Recharts ComposedChart', str_contains($chartSource, 'ComposedChart'));
    check('Chart includes a stockout ReferenceLine', str_contains($chartSource, 'ReferenceLine') && str_contains($chartSource, 'y={0}'));
    check('SKU detail page renders DemandForecastChart', str_contains($skuSource, '<DemandForecastChart'));
    check('Dashboard renders Daily Summary card', str_contains($dashboardSource, 'Day {dailySummary.created_day} Summary'));
