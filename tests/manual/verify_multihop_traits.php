...
try {
    ...
    if ($count !== 3) {
        throw new Exception("Expected 3 locations, found {$count}");
    }
    ...
    if (! $r1 || ! $r2) {
        throw new Exception('Routes not created correctly');
    }
    if ($r1->transit_days != 2 || $r1->cost != 50) {
        throw new Exception('Route 1 attributes mismatch');
    }
    ...
    if (! $pivot) {
        throw new Exception('Product-Vendor link not created');
    }

    // Check product price
    if ((float) $prod->unit_price !== 15.0) {
        throw new Exception('Product unit_price mismatch: '.$prod->unit_price);
    }
    ...
    if ($gs->cash != 500.0) {
        throw new Exception("GameState cash mismatch: {$gs->cash}");
    }
    if ($gs->user_id !== $user->id) {
        throw new Exception('GameState user mismatch');
    }
