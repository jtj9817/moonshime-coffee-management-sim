...
- [ ] Task: Implement `DemandForecastService`.
    - [ ] Write test for `getForecast` to verify projections based on base consumption and active spikes.
    - [ ] Implement `getForecast` logic to return a 7-day projection array.
- [ ] Task: Create `DemandForecastChart` frontend component.
    - [ ] Create `DemandForecastChart.tsx` using Recharts to display projected stock vs zero-line.
...
- [ ] Task: Implement daily summary aggregation.
    ...
    - [ ] Update simulation logic to aggregate units sold, lost sales, and total costs into a summary alert.
...
- [ ] Task: Implement `QuestService` and Trigger Architecture.
    ...
    - [ ] Implement `QuestService` to load active quests and execute trigger classes.
    - [ ] Implement core trigger types: Transactional, Financial, Resilience, Operational.
...
- [ ] Task: Implement Scenario Calculator backend endpoint/logic.
    - [ ] Write test for `ScenarioCalculatorService` to verify time-to-stockout and reorder recommendations.
    - [ ] Implement logic (sharing math with `DemandForecastService`).
- [ ] Task: Scenario Calculator UI components.
    - [ ] Create `ScenarioPlanner.tsx` standalone tool.
...
- [ ] Task: Create migration and model for `ScheduledOrder`.
    - [ ] Write migration for `scheduled_orders` table.
- [ ] Task: Implement Order Scheduler logic.
    ...
    - [ ] Implement logic to process active schedules and create/submit orders.
