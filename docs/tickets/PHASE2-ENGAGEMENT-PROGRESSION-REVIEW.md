...
1.  **Refactor `QuestService`**:
    -   Remove `is_completed` update logic from `getActiveQuestsForUser`.
    -   Ensure `checkTriggers` remains the sole authority for completing quests and granting rewards.
2.  **Implement Mitigation Endpoint**:
    -   Add `mitigate` method to `GameController`.
    -   Register route in `routes/web.php`.
3.  **Enhance Data Integrity**:
    -   Add transactions to `SpikeResolutionService::mitigate`.
