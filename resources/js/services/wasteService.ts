  ...
  for (let i = 0; i < diffDays; i++) {
    ...
    if (Math.random() < 0.4) {
       ...
       if (isPerishable) {
         ...
         if (rand < 0.4) reason = 'SUPPLIER_DELAY'; // Damaged in transit etc
         else reason = 'OTHER';
       }

       // Qty dependent on item cost/size
       const baseQty = item.category === 'Milk' ? 5 : item.category === 'Pastry' ? 12 : 2;
       const qty = Math.max(1, Math.floor(baseQty + (Math.random() * baseQty)));
  ...
  for (let i = 0; i < diffDays; i++) {
    ...
    if (Math.random() < 0.1) {
       ...
       logs.push({
         ...
         newValue: newV,
         reason,
         impactMetric: Math.random() > 0.5 ? 'Projected Savings: $45/mo' : undefined
       });
    }
  }
