...
export const generateAlerts = (
    ...
    if (relevantInventory.length > 0 && Math.random() > 0.7) {
        const randomRec =
            relevantInventory[
                Math.floor(Math.random() * relevantInventory.length)
            ];
...
export const generateSuggestedActions = (
    ...
    alerts
        .filter((a) => a.type === 'STOCKOUT')
        .forEach((a) => {
            locationCounts[a.locationId] =
                (locationCounts[a.locationId] || 0) + 1;
        });
    ...
    Object.entries(locationCounts).forEach(([locId, count]) => {
        if (count >= 2) {
            actions.push({
                ...
                impact: { savings: 45.0 },
                action: {
                    label: 'Start Consolidated Order',
                    to: `/ordering?locId=${locId}`,
                },
            });
        }
    });
    ...
    const criticalStockout = alerts.find(
        (a) => a.type === 'STOCKOUT' && a.severity === 'critical',
    );
    if (criticalStockout) {
        actions.push({
            ...
            action: {
                ...
                to: `/inventory?loc=${criticalStockout.locationId}`,
            },
        });
    }
...
export const suggestConsolidationAdds = (
    ...
    const currentSubtotal = draft.items.reduce(
    ...
    const availableItems = SUPPLIER_ITEMS.filter(
        (si) => si.supplierId === supplier.id,
    );
    ...
    availableItems.forEach((si) => {
        ...
        const pos = inventoryPositions.find(
            (p) => p.skuId === si.itemId && p.locationId === locationId,
        );
