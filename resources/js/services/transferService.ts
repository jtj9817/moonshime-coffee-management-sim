...
const TRANSFER_FIXED_COST = 15.0; // e.g. Courier fee
...
export const generateTransferSuggestions = (
    ...
    inventory.forEach((r) => {
        locationStorageUsage[r.locationId] =
            (locationStorageUsage[r.locationId] || 0) + r.quantity;
    });
    ...
    receivers.forEach((receiver) => {
        ...
        let neededQty =
            Math.ceil(receiver.reorderPoint * 1.2) - receiver.onHand;
        ...
        const supplierItems = SUPPLIER_ITEMS.filter(
            (si) => si.itemId === receiver.skuId,
        );
        if (supplierItems.length === 0) return;

        // Simplification: Average supplier price + shipping amortized (roughly)
        const avgSupplierPrice =
            supplierItems.reduce((acc, curr) => acc + curr.pricePerUnit, 0) /
            supplierItems.length;
        const avgSupplierLeadTime =
            supplierItems.reduce((acc, curr) => acc + curr.deliveryDays, 0) /
            supplierItems.length;
        ...
        const potentialDonors = positions.filter(
            (p) =>
        ...
        potentialDonors.forEach((donor) => {
            ...
            const timeSaved = Math.max(
                0,
                avgSupplierLeadTime - TRANSFER_LEAD_TIME_DAYS,
            );
