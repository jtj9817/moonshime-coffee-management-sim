...
export const calculateSafetyStock = (
    ...
    const combinedUncertainty = Math.sqrt(
        avgLeadTime * demandVariance +
            Math.pow(avgDailyUsage, 2) * leadTimeVariance,
    );
...
export const evaluateBulkTierBreakeven = (
    ...
    const incrementalHoldingCost =
        item.storageCostPerUnit * extraUnits * estimatedMonthsToConsumeExtra;
    ...
    let recommendation: 'UPGRADE_TIER' | 'STAY_CURRENT' | 'NOT_WORTH_IT' =
        'NOT_WORTH_IT';

    if (exceedsStorage) {
        recommendation = 'NOT_WORTH_IT'; // Storage risk
    } else if (netBenefit > 50) {
...
export const generateMockForecast = (
    ...
    for (let i = 0; i < days; i++) {
        ...
        const predicted = Math.max(
            0,
            Math.floor(baseDemand * dowFactor * seasonalityMultiplier + noise),
        );
