...
export interface OrderWarning {
  kind: 'EXPIRY' | 'TIER_BAD_DEAL' | 'STOCKOUT_RISK' | 'MIN_ORDER' | 'CONSOLIDATION_OPP';
  message: string;
...
export type TransferStatus = 'DRAFT' | 'PENDING_APPROVAL' | 'IN_TRANSIT' | 'COMPLETED' | 'CANCELLED';
...
export interface Quest {
  ...
  isCompleted: boolean;
  type: 'STOCK_LEVEL' | 'NO_ALERTS' | 'TRANSFER_EFFICIENCY' | 'VENDOR_NEGOTIATION';
  targetValue?: number;
  currentValue?: number;
  locationId?: string;
  itemId?: string;
...
export interface AppContextType {
  ...
  placeOrder: (locId: string, item: Item, qty: number, supplier: Supplier) => void;

  // Cart
  drafts: DraftOrder[];
  addToDraft: (vendorId: string, locationId: string, item: Item, qty: number, price: number) => void;
  ...
  // Transfers
  transfers: Transfer[];
  createTransfer: (sourceId: string, targetId: string, items: TransferLineItem[]) => void;
  ...
  negotiateWithVendor: (supplierId: string) => Promise<{ success: boolean; discount?: number }>;
  triggerFeedback: (text: string, type: 'positive' | 'negative' | 'neutral' | 'xp') => void;
...
export type AlertType = 'STOCKOUT' | 'EXPIRY' | 'SPIKE' | 'VENDOR_DELAY' | 'WASTE';
...
export type SuggestedActionKind = 'PLACE_ORDER' | 'TRANSFER' | 'ADJUST_REORDER_POINT' | 'CONSOLIDATE_CART';
...
export type InventoryStatusCode = 'OK' | 'BELOW_ROP' | 'EXPIRY_RISK' | 'STOCKOUT_RISK' | 'SPIKE_RISK';
...
export interface SpikeHistoryEvent {
  ...
  rootCause: 'WEATHER' | 'LOCAL_EVENT' | 'PROMOTION' | 'SUPPLY_FAILURE' | 'UNKNOWN';
...
export interface SpikeFeedback {
  ...
  classification: 'REAL_DEMAND' | 'DATA_ERROR' | 'ONE_OFF_EVENT' | 'OPERATIONAL_WASTE';
  rootCause?: 'WEATHER' | 'LOCAL_EVENT' | 'PROMOTION' | 'SUPPLY_FAILURE' | 'UNKNOWN';
  notes: string;
}
...
export type WasteReason = 'OVER_ORDER' | 'FORECAST_MISS' | 'SUPPLIER_DELAY' | 'EXPIRY' | 'OTHER';
...
export interface PolicyChangeLog {
  ...
  skuId?: string;
  locationId?: string;
  changeType: 'REORDER_POINT' | 'SAFETY_STOCK' | 'SUPPLIER_CHANGE' | 'TRANSFER_RULE';
