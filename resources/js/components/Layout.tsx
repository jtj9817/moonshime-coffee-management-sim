...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  const { locations, currentLocationId, setCurrentLocationId, notifications, markNotificationRead, gameState } = useApp();
                              ...
                              {notifications.length === 0 ? (
                                ...
                                notifications.map(n => (
                                  <div 
                                    ...
                                    className={`p-4 border-b border-stone-800 hover:bg-stone-800/50 cursor-pointer transition-colors ${n.read ? 'opacity-50' : 'bg-stone-800/30'}`}
                                  >
                                     <div className="flex gap-3">
                                        <div className={`w-1.5 h-1.5 mt-1.5 rounded-full flex-shrink-0 ${n.type === 'alert' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]' : n.type === 'info' ? 'bg-blue-500' : 'bg-emerald-500'}`}></div>
                                        <div>
                                           <p className="text-xs font-bold text-stone-200 leading-snug">{n.message}</p>
                                           <p className="text-[10px] text-stone-500 mt-1 font-mono">{new Date(n.timestamp).toLocaleTimeString()}</p>
                                        </div>
                                     </div>
                                  </div>
                                ))
